...
class YoutubeScraper:
    ...
    def validate_data_before_upload(self, new_data):
        ...
        for channel_name, new_df in new_data.items():
            if channel_name not in existing_data:
                continue
            
            old_df = existing_data[channel_name]
            ...
            if new_df.empty or len(new_df.columns) == 0:
                issues.append(f"@{channel_name}: New data is empty")
                continue
            ...
            try:
                ...
                # Allow tolerance - 90% threshold
                min_acceptable = int(old_videos * 0.9)
                
                if new_videos < min_acceptable:
                    issues.append(f"@{channel_name}: New scrape has {new_videos} videos vs {old_videos} previously (below 90% threshold)")
            except Exception as e:
                print(f"  ⚠️  Could not compare videos for @{channel_name}: {e}")
